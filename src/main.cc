...
int main(void)
{
    ...
    // ------------------------------------------------------------------
    int size = 100;
    float width_rec = 0.5f;
    auto vertices = init_waves(size, width_rec);
    auto indices = init_indices(size);
...
// load and create a texture
    ...
    unsigned char *data = stbi_load("resources/reflection.jpg", &width, &height, &nrChannels, 0);
    if (data)
    {
